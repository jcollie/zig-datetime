...
// milliseconds, the format string, and what this library produced -- and
// this asks moment the same question and reports every answer that
// differs. It exits non-zero when any do, so `zig build oracle` fails on
...
// build.zig. Everything is compared in UTC.
...
for (const line of lines) {
    // Only the first two tabs separate fields; a formatted result may
    ...
    const format = line.slice(first + 1, second);
    const ours = line.slice(second + 1);

    const theirs = moment.utc(at).format(format);
    checked += 1;

    if (ours !== theirs) {
        mismatches.push({ at, format, ours, theirs });
    }
}
